...
def home():
    ...
    return render_template('home.html', items=items, cart=Cart.query.filter_by(customer_link=current_user.id).all()
                           if current_user.is_authenticated else [])  #This function displays the number of items in cart
...
def add_to_cart(item_id):
    ...
    if item_exists:
        try:
            ...
            db.session.commit()
            flash(f'Quantity of {item_exists.product.product_name} has been updated successfully')
            return redirect(request.referrer) #redirecting back home
        except Exception as e:
            print('Item not updated', e)
            flash(f'Quantity of {item_exists.product.product_name} has not been updated')
            return redirect(request.referrer) #redirecting back home
...
def place_order():
    ...
    if customer_cart:
        try:
            ...
            create_order_response = service.collect.mpesa_stk_push(phone_number='0714431840 ', email=current_user.email,
